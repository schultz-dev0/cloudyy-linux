#!/usr/bin/env bash
# Ollama service control for Command Center (ported from rofi/ai.sh).
set -uo pipefail
...
service_menu() {
  ...
  choice=$(printf '%b' \
    "▶ Start Service\n■ Stop Service\n Restart Service\n󰋼 Service Status" |
    rofi -dmenu -i -p "Ollama Service (${status})") || exit 0
...
case "$cmd" in
  ...
  status) ollama_status_label ;;
  *)
    echo "usage: ollama-service.sh [menu|status]" >&2
    exit 1
    ;;
esac
