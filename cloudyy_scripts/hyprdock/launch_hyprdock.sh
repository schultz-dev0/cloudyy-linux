...
# bug, but it ensures hyprdock launches if systemd is broken.
launch_fallback() {
  log_info "Attempting fallback launch (setsid)..."
  (
    unset XDG_ACTIVATION_TOKEN DESKTOP_STARTUP_ID
    setsid "${APP_PATH}" "$@" </dev/null >/dev/null 2>&1 &
  )
...
[[ -x "${APP_PATH}" ]] || {
  log_err "${APP_NAME} binary not found at ${APP_PATH}"
  exit 1
}
[[ -d ${XDG_RUNTIME_DIR:-} ]] || {
...
if command -v systemd-run >/dev/null 2>&1; then
  ...
  if systemd-run --user --quiet --unit="${unit_name}" -- "${APP_PATH}" "$@" >/dev/null 2>&1; then
