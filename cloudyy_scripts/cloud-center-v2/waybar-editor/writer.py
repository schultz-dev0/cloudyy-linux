...
def update_css_property(css: str, selector: str, prop: str, value: str) -> str:
    ...
    prop_re = re.compile(r"(\b" + re.escape(prop) + r"\s*:\s*)([^;]+)(;)")
...
def save_config(preset: Preset) -> None:
    ...
    def module_names(modules: list[WaybarModule]) -> list:
        result = []
        for m in modules:
            if not m.enabled:
                continue
    ...
    cfg["modules-left"]   = module_names(preset.modules_left)
    cfg["modules-center"] = module_names(preset.modules_center)
    cfg["modules-right"]  = module_names(preset.modules_right)

    _atomic_write(preset.config_path, json.dumps(cfg, indent=4))
