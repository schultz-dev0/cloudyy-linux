# Rules & Startup Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a "Rules & Startup" tabbed page to Cloud Center with four sections (Window Rules, Layer Rules, Autostart, Environment Variables), each as a list editor with add/edit/delete dialogs and live-apply via `hyprctl reload`.

**Architecture:** Single module `lib/rules_startup_page.py` with data models, parse/serialize functions, four tab classes, four dialog classes, a window picker, an app picker, and the parent `RulesStartupPage`. One managed conf file at `~/.config/hypr/user-configs/user_rules_startup.conf` with four marker-delimited sections. Any mutation writes the file and runs `hyprctl reload` immediately; Apply commits the baseline; Discard reverts to it.

**Tech Stack:** Python 3.14, GTK4, Libadwaita (PyGObject), subprocess for hyprctl, pytest 9 for data-layer tests.

**Spec:** `docs/superpowers/specs/2026-05-07-rules-startup-page-design.md`

---

### Task 1: Test scaffold and data models

**Files:**
- Create: `tests/__init__.py`
- Create: `tests/test_rules_startup.py`
- Create: `lib/rules_startup_page.py` (constants + dataclasses only)
...
- [ ] **Step 2: Write failing tests for data model equality**
...
python3 -m pytest tests/test_rules_startup.py -v
```

Expected: `ModuleNotFoundError: No module named 'lib.rules_startup_page'`

- [ ] **Step 4: Create `lib/rules_startup_page.py` with constants and data models**
...
```bash
git add tests/__init__.py tests/test_rules_startup.py lib/rules_startup_page.py
git commit -m "feat: add Rules & Startup data models and test scaffold"
```

---

### Task 2: Window rule parse/serialize

**Files:**
- Modify: `lib/rules_startup_page.py` — add `_parse_window_rules`, `_serialize_window_rules`
- Modify: `tests/test_rules_startup.py` — add parse/serialize tests

- [ ] **Step 1: Write failing tests**

Append to `tests/test_rules_startup.py`:

```python
from lib.rules_startup_page import _parse_window_rules, _serialize_window_rules
...
```bash
python3 -m pytest tests/test_rules_startup.py -v -k "window"
```

Expected: `ImportError: cannot import name '_parse_window_rules'`

- [ ] **Step 3: Add to `lib/rules_startup_page.py` after the dataclasses**
...
```bash
git add lib/rules_startup_page.py tests/test_rules_startup.py
git commit -m "feat: add window rule parse/serialize"
```

---

### Task 3: Layer rule parse/serialize

**Files:**
- Modify: `lib/rules_startup_page.py` — add `_parse_layer_rules`, `_serialize_layer_rules`
- Modify: `tests/test_rules_startup.py`
...
```python
from lib.rules_startup_page import _parse_layer_rules, _serialize_layer_rules
...
```bash
python3 -m pytest tests/test_rules_startup.py -v -k "layer"
```

- [ ] **Step 3: Add to `lib/rules_startup_page.py`**
...
```bash
git add lib/rules_startup_page.py tests/test_rules_startup.py
git commit -m "feat: add layer rule parse/serialize"
```

---

### Task 4: Autostart and env var parse/serialize

**Files:**
- Modify: `lib/rules_startup_page.py`
- Modify: `tests/test_rules_startup.py`
...
```bash
python3 -m pytest tests/test_rules_startup.py -v -k "autostart or env"
```
...
```bash
git add lib/rules_startup_page.py tests/test_rules_startup.py
git commit -m "feat: add autostart and env var parse/serialize"
```

---

### Task 5: Conf file I/O

**Files:**
- Modify: `lib/rules_startup_page.py` — add `_parse_conf`, `_write_conf`
- Modify: `tests/test_rules_startup.py`
...
```bash
python3 -m pytest tests/test_rules_startup.py -v -k "conf"
```
...
Expected: all tests pass (≈24 at this point)

- [ ] **Step 5: Commit**

```bash
git add lib/rules_startup_page.py tests/test_rules_startup.py
git commit -m "feat: add conf file I/O for rules_startup"
```

---

### Task 6: Window Rule dialog and Window Picker dialog

No unit tests for GTK classes — manual smoke test in Task 11.

**Files:**
- Modify: `lib/rules_startup_page.py` — add `_gtk_imports`, `_WindowPickerDialog`, `_WindowRuleDialog`

- [ ] **Step 1: Add GTK import helper and `_WindowPickerDialog`**

Append to `lib/rules_startup_page.py`:
...
- [ ] **Step 2: Add `_WindowRuleDialog`**

Append after `_WindowPickerDialog`:
...
class _WindowRuleDialog:
    ...
    def __init__(self, parent_widget, on_save, existing: Optional[WindowRule] = None) -> None:
        ...
        Gtk.Button(label='Cancel').connect('clicked', lambda _: self._dialog.close())
...
```bash
git add lib/rules_startup_page.py
git commit -m "feat: add window rule and window picker dialogs"
```

---

### Task 7: Layer Rule dialog

**Files:**
- Modify: `lib/rules_startup_page.py` — add `_LayerRuleDialog`

- [ ] **Step 1: Add `_LayerRuleDialog`**
...
```bash
git add lib/rules_startup_page.py
git commit -m "feat: add layer rule dialog"
```

---

### Task 8: App Picker, Autostart, and Env Var dialogs

**Files:**
- Modify: `lib/rules_startup_page.py` — add `_AppPickerDialog`, `_AutostartDialog`, `_EnvVarDialog`

- [ ] **Step 1: Add `_AppPickerDialog`**
...
```bash
git add lib/rules_startup_page.py
git commit -m "feat: add app picker, autostart, and env var dialogs"
```
...
**Files:**
- Modify: `lib/rules_startup_page.py` — add `_make_list_header`, `_make_rule_row`, and four tab classes

- [ ] **Step 1: Add shared row-building helpers**
...
- [ ] **Step 2: Add all four tab classes**
...
```bash
git add lib/rules_startup_page.py
git commit -m "feat: add all four tab classes for rules_startup"
```

---

### Task 10: RulesStartupPage — parent page, footer, apply logic

**Files:**
- Modify: `lib/rules_startup_page.py` — add `RulesStartupPage`

- [ ] **Step 1: Add `RulesStartupPage` at the end of `lib/rules_startup_page.py`**

```python
# ── Rules & Startup Page ───────────────────────────────────────────────────────

class RulesStartupPage(Gtk.Box):
...
```bash
git add lib/rules_startup_page.py
git commit -m "feat: add RulesStartupPage with apply/discard logic"
```

---

### Task 11: Register in cloud-center.py and smoke test

**Files:**
- Modify: `cloud-center.py`

- [ ] **Step 1: Add import**

Find the import block near line 57 (`import lib.keybind_manager as keybind_manager`). Add:

```python
import lib.rules_startup_page as rules_startup_page
```

- [ ] **Step 2: Add builtin entry**

In `_build_sidebar()`, find the `builtins` dict (around line 299). Add:

```python
"__rules__": {"id": "__rules__", "title": "Rules & Startup", "icon": "preferences-system-symbolic"},
```

- [ ] **Step 3: Add to Visuals category**

Find the `categories` list (around line 308). Change the Visuals entry to include `"__rules__"` after `"hyprland"`:

```python
("Visuals", ["appearance", ACTIVE_SHELL_TAB, "hyprland", "__rules__"]),
```

- [ ] **Step 4: Add lazy builder**

Find `self._lazy_builders` dict (around line 451). Add:
...
1. "Rules & Startup" appears in the sidebar under Visuals
2. Clicking it loads the page with Window Rules tab active
3. All four tabs are navigable
4. "Add" on Window Rules tab opens the dialog with name, matcher row, effects toggles, and live preview
5. "Pick" button in matcher row opens the window picker (shows running windows or "Could not list windows")
6. Toggling an effect in the dialog updates the preview block
7. Adding a rule closes the dialog, shows the rule in the list, enables Apply/Discard buttons
8. Clicking Apply shows a toast and disables Apply/Discard
9. Relaunching the app reloads the saved rules
10. Edit opens the dialog pre-filled with the existing rule's values
11. Delete removes the rule from the list and triggers live apply
12. Autostart tab: Add opens dialog with command entry, app picker Browse button, and exec-once toggle
13. Environment tab: Add validates name field (invalid names show error, Save stays disabled)
14. Discard reverts all tabs to last confirmed state
...
git commit -m "feat: register Rules & Startup page in Cloud Center sidebar"
```
