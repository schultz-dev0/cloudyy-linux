...
[[ ! -f "$DEPS_FILE" ]] && {
  log_error "dependencies.conf not found: ${DEPS_FILE}"
  exit 1
}
source "$DEPS_FILE"
...
setup_aur_helper() {
  ...
  local choice
  read -rp "  Selection [1-3, Enter=1]: " choice
  choice="${choice:-1}"
...
# -----------------------------------------------------------------------------
# write_gpu_launcher
# Generates ~/.config/hypr/cloudyy-launch.sh with the correct environment
# variables for the chosen GPU, then registers it as the Hyprland launcher.
# Also injects an env block into hyprland.conf so child processes inherit vars.
# Called from select_gpu() after the user confirms their GPU choice.
# -----------------------------------------------------------------------------
write_gpu_launcher() {
  local vendor="$1" # "nvidia" | "amd" | "intel" | "skip"
...
LAUNCHER_EOF
  ...
  case "$vendor" in
  nvidia)
    # Resolve the first available DRM card node at launch time so the
    # script stays correct even if card indices shift between reboots.
    cat >>"$launcher" <<'NVIDIA_EOF'
# ── NVIDIA ────────────────────────────────────────────────────────────────────
...
NVIDIA_EOF
    ;;
  amd)
    cat >>"$launcher" <<'AMD_EOF'
# ── AMD ───────────────────────────────────────────────────────────────────────
export LIBVA_DRIVER_NAME=radeonsi
export VDPAU_DRIVER=radeonsi
# AMD works cleanly with Mesa defaults — no GBM or EGL overrides needed.
AMD_EOF
    ;;
  intel)
    cat >>"$launcher" <<'INTEL_EOF'
# ── Intel ─────────────────────────────────────────────────────────────────────
export LIBVA_DRIVER_NAME=iHD      # intel-media-driver (Gen8+); change to i965 for older hardware
export VDPAU_DRIVER=va_gl
INTEL_EOF
    ;;
  *)
    cat >>"$launcher" <<'SKIP_EOF'
# ── GPU vendor not detected — launching with defaults ─────────────────────────
SKIP_EOF
    ;;
  esac

  # Universal Wayland/XDG vars that every GPU path needs
  cat >>"$launcher" <<'COMMON_EOF'
...
# cloudyy-gpu-env — injected by cloudyy-linux installer (nvidia)
...
# cloudyy-gpu-env — injected by cloudyy-linux installer (amd)
...
# cloudyy-gpu-env — injected by cloudyy-linux installer (intel)
...
HYPR_INTEL
    ...
    if [[ -n "$env_block" ]]; then
      printf '%s\n' "$env_block" >>"$hypr_conf"
      log_ok "GPU env block appended to hyprland.conf."
    fi
  elif [[ -f "$hypr_conf" ]]; then
    log_skip "GPU env block already present in hyprland.conf — skipping."
  else
    log_warn "hyprland.conf not found yet — env block will be added after dotfiles deploy."
  fi
}
...
select_gpu() {
  ...
  local gpu
  gpu="$(lspci 2>/dev/null | grep -iE 'vga|3d|display' | cut -d: -f3 | xargs 2>/dev/null)" || true
  local cpu
  cpu="$(lscpu 2>/dev/null | grep 'Model name' | cut -d: -f2 | xargs 2>/dev/null)" || true
  printf '  CPU : %s\n' "${cpu:-Unknown}"
  printf '  GPU : %s\n' "${gpu:-Unknown}"
  printf '\n'
