...
phase_laptop() {
  ...
  log "Laptop detected — configuring touchpad and input settings..."
...
TOUCHPAD
    ...
    log_ok "Touchpad config already present — skipping."
  fi

  # Source input-overrides.conf from hyprland.conf (idempotent)
  if [[ -f "$hypr_conf" ]] && ! grep -qF "input-overrides.conf" "$hypr_conf"; then
    printf '\n# cloudyy-laptop-input\nsource = %s\n' "$input_conf" >>"$hypr_conf"
    log_ok "input-overrides.conf registered in hyprland.conf."
  fi
