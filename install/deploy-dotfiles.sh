...
# Wire cloudyy bin/ onto PATH via /etc/profile.d (idempotent)
setup_cloudyy_path() {
  log_section "Shell PATH"

  local profile_d_file="/etc/profile.d/cloudyy.sh"
  local repo_dir="${REPO_DIR}"
  local desired_line="export PATH=\"\$PATH:${repo_dir}/bin\""

  if [[ -f "$profile_d_file" ]] && grep -Fxq "$desired_line" "$profile_d_file"; then
    log_skip "PATH already wired via ${profile_d_file}"
    return 0
  fi

  if sudo tee "$profile_d_file" >/dev/null <<EOF
# Auto-generated by cloudyy-linux deploy-dotfiles.sh — do not edit by hand.
${desired_line}
EOF
  then
    log_ok "Wired cloudyy bin/ onto PATH via ${profile_d_file}"
  else
    log_error "Failed to write ${profile_d_file} — PATH not wired, run cloudyy manually via ./bin/cloudyy"
    return 1
  fi
}
