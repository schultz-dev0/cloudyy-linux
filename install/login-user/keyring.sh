#!/usr/bin/env bash
# setup-keyring.sh — Installs PAM support for Cloudyy session unlocking
...
lock_pam="${HOME}/cloudyy-linux/install/assets/pam/cloudyy-lock"
if [[ -f "$lock_pam" ]]; then
  sudo install -Dm644 "$lock_pam" /etc/pam.d/cloudyy-lock
  log_ok "Cloudyy lockscreen PAM service installed."
fi
