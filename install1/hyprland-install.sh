...
echo ""
print_status "Installing ${#PACKAGES_TO_INSTALL[@]} packages from official repositories..."
echo ""
...
if [ -f "$DEPS_FILE" ] && [ ${#AUR_PACKAGES[@]} -gt 0 ]; then
    echo ""
    ...
    if [[ $aur_choice =~ ^[Yy]$ ]]; then
        ...
        for pkg in "${AUR_PACKAGES[@]}"; do
            if [ -n "$pkg" ]; then
                print_status "Installing AUR package: $pkg"
